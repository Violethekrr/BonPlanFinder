...
const tutorialSteps = [
  ...
  const getCheapestProducts = useMemo(() => {
    const productsWithPrices = produits.map(product => {
      ...
      const productCategory = catalogues.find(c => c.id === product.id_type);
      
      return {
        ...product,
        minPrice,
        categoryName: productCategory?.titre || 'Non catégorisé',
  ...
  const handleSearch = () => {
    ...
    const matchedCategory = catalogues.find(category => 
      category.titre.toLowerCase().includes(query)
    );
    ...
    if (matchedProducts.length > 0) {
      ...
      const productCategory = catalogues.find(c => c.id === firstProduct.id_type);
      
      // Naviguer vers la page des produits avec la catégorie du produit trouvé
      navigate('/Produits', { 
        state: { 
          id: firstProduct.id_type,
          nom: productCategory?.titre || 'Résultats de recherche'
        } 
      });
    } else {
  ...
  const handleSearchChange = useCallback((value: string) => {
    ...
    const categoryMatches = catalogues
      .filter(category => category.titre.toLowerCase().includes(query))
    ...
    const productMatches = produits
      ...
      .map(product => {
        ...
        return {
          ...
          id: product.id,
          nom: product.nom,
          categoryName: productCategory?.titre
        };
      });
  ...
  return (
    <div className={`pt-2 pb-10 space-y-10 min-h-screen ${isDark ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'}`}>
          ...
          >
            {catalogues.map((c, index) => (
              <motion.span
                ...
                onClick={() => goToProducts(c.id || 1, c.titre)}
      ...
      <section className="px-4 md:px-6 lg:px-8 py-8 md:py-14 space-y-8 md:space-y-10">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
          ...
          <motion.div 
            ...
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.6 }}
            className="pl-0 lg:pl-12  md:mt-14 lg:mt-0"
          >
            <h1 className={`text-xl md:text-2xl xl:text-3xl font-bold mb-3 md:mb-4 ${isDark ? 'text-white' : ''}`}>
              Notre Mission : Vous Faire Économiser
            </h1>

            <hr className="w-24 md:w-32 mb-4 md:mb-6 border-[#F47D1C] border-2 md:border-4" />

            <p className={`max-w-4xl text-sm md:text-base xl:text-lg mb-6 md:mb-10 leading-relaxed ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
              ...
              <div className="space-y-4 md:space-y-6">
                ...
                >
                  ...
                  <div>
                    ...
                    <p className={`text-sm md:text-base xl:text-lg ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
      ...
      >
        <div className="flex justify-center">
          <div className="text-center">
            <h1 className={`font-bold text-xl md:text-2xl xl:text-3xl ${isDark ? 'text-white' : ''}`}>Bonnes Affaires du Moment</h1>
        ...
        <div className="flex justify-center pt-4 md:pt-6">
          <motion.button 
            ...
            className={`px-6 md:px-8 py-3 text-sm md:text-base xl:text-lg md:py-4 rounded-xl font-semibold transition-all duration-300 shadow-lg hover:shadow-xl ${
      ...
      >
        <div className="flex justify-center">
          <div className="text-center">
            <h1 className={`font-bold text-xl md:text-2xl xl:text-3xl ${isDark ? 'text-white' : ''}`}>
        ...
        <div className="text-center">
          <p className={`text-sm md:text-base xl:text-lg leading-relaxed ${isDark ? 'text-gray-300' : 'text-[#063A5F]'}`}>
        ...
        <div className="flex flex-col sm:flex-row gap-4 md:gap-8 justify-center items-center">
          <motion.button 
            ...
            whileTap={{ scale: 0.95 }}
            onClick={goToCatalogues}
            className={`rounded-xl px-6 py-3 text-sm md:text-base xl:text-lg md:px-8 md:py-4 font-bold transition-all duration-300 shadow-lg hover:shadow-xl w-full sm:w-auto ${
              isDark ? 'bg-[#e56d0c] hover:bg-[#d55c00]' : 'bg-[#F47D1C] hover:bg-[#e56d0c]'
            } text-white`}
          >
            Voir les catalogues
          ...
          <motion.button 
            ...
            className={`rounded-xl text-sm md:text-base xl:text-lg font-bold text-white px-6 py-3 md:px-8 md:py-4 transition-all duration-300 shadow-lg hover:shadow-xl w-full sm:w-auto ${
              isDark ? 'bg-[#4a8a2d] hover:bg-[#3a7a1d]' : 'bg-[#5AA136] hover:bg-[#4a8a2d]'
            }`}
          >
